...
var theater = ["SNH48", "BEJ48", "GNZ48"];
var teamList = ["Team SII", "Team NII", "Team HII", "Team X", "Team B", "Team E",
    "Team J", "Team G", "Team NIII", "Team Z"];
// member
var memberSII = ["陈观慧", "陈俊羽", "陈思", "戴萌", "蒋芸", "孔肖吟", "李宇琪", "刘增艳", "莫寒", "钱蓓婷", "邱欣怡",
    "孙芮", "邵雪聪", "温晶婕", "吴哲晗", "徐晨辰", "许佳琪", "徐子轩", "袁丹妮", "杨令仪", "袁雨桢", "朱小丹", "张语格"];
...
var sampleTime;
var growthTotal = 0, growthTheater, growthTeam, growthMember;

// By default, lines in selectStatus with value=false are not displayed.
var selectStatus = {
    "SNH48": true,
    "BEJ48": true,
    "GNZ48": true,
};
// By default, initialized data of each team are all 0.
var growthDataTeam = {};
// Create complete selectStatus and growthDataTeam.
for (var i=0; i<teamList.length; i++) {
    var selectStatusSubTeam = {};
    var growthDataTeamSub = {};

    selectStatusSubTeam[teamList[i]] = false;     // {team: false}
    growthDataTeamSub[teamList[i]] = [0];   // {team: [0]}

    $.extend(selectStatus, selectStatusSubTeam);
    $.extend(growthDataTeam, growthDataTeamSub);
}

// By default, initialized data of each member are all 0.
var growthDataMember = {};
// Create complete selectStatus and growthDataMember.
for (var i=0; i<memberList.length; i++) {
    var selectStatusSub = {};
    var growthDataMemberSub = {};

    selectStatusSub[memberList[i]] = false;     // {member: false}
    growthDataMemberSub[memberList[i]] = [0];   // {member: [0]}

    $.extend(selectStatus, selectStatusSub);
    $.extend(growthDataMember, growthDataMemberSub);
}
...
function dropDownCk(selectId,hiddenId, item) {
    var boxId = "#" + boxId,
        selectId = "#" + selectId,
        hiddenId = "#" + hiddenId;
...
function addOption(selectedTeam, selectedMember) {
    ...
    for (var i=0; i<selectedTeam.length; i++) {
        var seriesSubTeam = {
            ...
            itemStyle: {
                normal: {
                    color: teamColor[selectedTeam[i]],
                    borderColor: teamColor[selectedTeam[i]],
                    areaStyle: {
                        type: 'default',
                        opacity: 0.0    //Color under lines.
                    }
    ...
    var selectedSubTeam = {};
    var teamRest = sub(teamList, selectedTeam);     // Teams which are not selected.
    for (var i=0; i<teamRest.length; i++) {
    ...
    for (var i=0; i<selectedMember.length; i++) {
        var seriesSub = {
            ...
            itemStyle: {
                normal: {
                    color: memberColor[selectedMember[i]],
                    borderColor: memberColor[selectedMember[i]],
                    areaStyle: {
                        type: 'default',
                        opacity: 0.0    //Color under lines.
                    }
    ...
    var selectedSub = {};
    var memberRest = sub(memberList, selectedMember);
    for (var i=0; i<memberRest.length; i++) {
...
setInterval(function () {
    ...
    // growth
    addData(true, sampleTime, growthTotal, growthTheater, growthTeam, growthMember);

    myChart.setOption(addOption(selectedTeam, selectedMember));
